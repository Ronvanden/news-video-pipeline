"""BA 27.4 — CLI: Reference Provider Payload Stubs (patch asset_manifest.json, no live calls)."""
...
from app.visual_plan.reference_provider_payloads import apply_reference_provider_payloads_to_manifest
...
def main() -> int:
    ap = argparse.ArgumentParser(description="BA 27.4 — Reference Provider Payloads (asset_manifest.json)")
    ...
    try:
        ...
        patched = apply_reference_provider_payloads_to_manifest(man)
        out_path = args.output if args.output is not None else args.manifest
        if not bool(args.dry_run):
            write_json(out_path, patched)
        summ = patched.get("reference_provider_payload_summary") if isinstance(patched.get("reference_provider_payload_summary"), dict) else {}
    ...
    stdout = {
        ...
        "not_supported_count": int(summ.get("not_supported_count") or 0),
        "none_count": int(summ.get("none_count") or 0),
        "providers_prepared": summ.get("providers_prepared") or {},
