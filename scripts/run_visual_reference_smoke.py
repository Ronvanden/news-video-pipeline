"""BA 32.73 — Visual Reference Smoke (1 Master Reference Image, OpenAI Image 2).
...
from app.production_connectors.visual_reference_layer import run_visual_reference_image_v1


RESULT_NAME = "visual_reference_smoke_result.json"


def main() -> int:
    p = argparse.ArgumentParser(
        description="BA 32.73 — Visual Reference Smoke: 1 Master Reference Image (gpt-image-2 Preferred Path). "
    ...
    p.add_argument(
        ...
        action="store_true",
        dest="confirm",
        help="Pflicht: 1 echter OpenAI Images API Call (Kostenfreigabe).",
    )
    ...
    rid = (args.run_id or "").strip() or f"vr_{uuid.uuid4().hex[:12]}"
    work_dir = Path(args.out_root).resolve() / f"visual_reference_smoke_{rid}"
    ...
    # Live run: dry_run=False. No auto-fallback: model is fixed by effective_model.
    body = run_visual_reference_image_v1(
        output_dir=work_dir,
