"""BA 25.5 — URL-to-Final-Video Smoke (lokal, one-command).
...
URL
  ...
  → BA 25.5: schreibt local_preview_<run_id>/ Snapshot + human_approval.json (Smoke-only)
...
RESULT_SCHEMA = "ba_25_5_url_to_final_video_result_v1"
...
def _default_run_id() -> str:
    # deterministic-ish, safe chars only
...
def _ensure_local_preview_package_from_ba254(
    *,
    ...
    """
    BA 25.5 Adapter-Schritt:
    Final Render (BA 24.x) erwartet `output/local_preview_<run_id>/...`.
    BA 25.4 liefert jedoch `output/real_local_preview_<run_id>/...` und eine Preview unter
    `output/subtitle_burnin_<run_id>/preview_with_subtitles.mp4`.

    Wir bauen ein minimales, lokales Preview-Paket (Smoke-only), ohne bestehende Contracts zu ändern.
    """
    paths: Dict[str, str] = {
        "local_preview_dir": "",
        "local_preview_video": "",
    ...
    # Minimal snapshot, damit BA 24.2 Dry-Run Gates nicht unknown bleiben.
    snap = {
        ...
        "founder_quality_decision": {
            ...
            "top_issue": "",
            "next_step": "BA 25.5 smoke created this snapshot; review preview manually.",
        },
        "production_costs": {"estimated_total_cost": 0.0, "over_budget_flag": False},
        "warnings": [
            "BA 25.5 smoke: local_preview_result.json ist ein minimales Snapshot-Stub für Final Render Gates.",
            f"BA 25.5 smoke: source_real_local_preview_result={real_local_preview_result_path}",
        ],
...
def _write_human_approval(
    *,
    ...
    """
    BA 25.5 Smoke-only: schreibt `human_approval.json` als approved, damit BA 24.x Final Render
    nicht in "locked" bleibt.
    """
    doc = {
    ...
    _write_json(p, doc)
    warnings.append("ba25_5_auto_approved_for_smoke")
...
def run_url_to_final_video_smoke(
    *,
    ...
    approval_note: str = "BA 25.5 smoke auto-approval (review required before publishing).",
    ...
    Schreibt ein additives Ergebnis-JSON: `url_to_final_video_result.json`.
    """
    rid = (run_id or "").strip()
    if not rid:
        rid = _default_run_id()
    ...
    if not isinstance(bridge_res, dict) or not bridge_res.get("ok"):
        ...
        result = {
            ...
            "run_id": rid,
            "url": url,
            "steps": {"ba25_3_url_to_script": bridge_res},
    ...
    if not isinstance(preview_res, dict):
        preview_res = {"ok": False, "status": "failed", "blocking_reasons": ["preview_invalid_result"]}
    ...
    paths["real_local_preview_result"] = str(Path(paths["real_local_preview_dir"]) / "real_local_preview_result.json") if paths["real_local_preview_dir"] else ""
    ...
    if not preview_res.get("ok") or not paths["preview_with_subtitles"]:
        ...
        result = {
            "schema_version": RESULT_SCHEMA,
            "ok": False,
            "status": "blocked" if str(preview_res.get("status") or "").lower() == "blocked" else "failed",
            "run_id": rid,
            "url": url,
            "steps": {"ba25_3_url_to_script": bridge_res, "ba25_4_real_local_preview": preview_res},
            "warnings": list(dict.fromkeys(warnings)),
            "blocking_reasons": list(dict.fromkeys(blocking)),
            "paths": paths,
            "exit_code": EXIT_BLOCKED if "blocked" in str(preview_res.get("status") or "").lower() else EXIT_FAILED,
            "created_at_epoch": int(time.time()),
        }
        _write_json(Path(paths["result_json"]), result)
        return result

    # 3) BA 25.5: build `local_preview_<run_id>` + approval for BA 24.x gates
    ...
    if auto_approve and paths["local_preview_dir"]:
        ...
        warnings.append("ba25_5_human_approval_not_written")

    # 4) BA 24.3 Final Render (copy preview)
    ...
    # `run_final_render.py` writes `final_render_result.json` under final dir
    ...
    ok = bool(final_res.get("ok")) and _safe_nonempty_file(Path(paths["final_video"])) if paths["final_video"] else bool(final_res.get("ok"))
    status = "completed" if ok else (str(final_res.get("status") or "failed"))

    exit_code = EXIT_OK if ok else (EXIT_BLOCKED if status in ("locked", "blocked", "unknown") else EXIT_FAILED)
    result = {
        "schema_version": RESULT_SCHEMA,
        "ok": ok,
        "status": status,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "BA 25.5 — URL-to-Final-Video Smoke (lokal): URL → generate_script_response.json → "
            "preview_with_subtitles.mp4 → final_video.mp4. Kein Publishing."
        )
    )
    ...
    p.add_argument("--no-auto-approve", action="store_true", dest="no_auto_approve")
    p.add_argument("--approval-note", default="", dest="approval_note")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    res = run_url_to_final_video_smoke(
        ...
        or "BA 25.5 smoke auto-approval (review required before publishing).",
    )
    ...
    if args.print_json:
        ...
        compact = {
            ...
            "final_video": (printable.get("paths") or {}).get("final_video", ""),
            "result_json": (printable.get("paths") or {}).get("result_json", ""),
