...
**Dashboard (BA 22.0–22.2):** Mit laufender FastAPI-App liefert **GET `/founder/dashboard/local-preview/panel`** eine read-only-Übersicht der Ordner **`local_preview_*`** unter dem Repository-**`output/`** (Artefakt-Flags, keine Dateiinhalte). **BA 22.1** ergänzt **Status-Karten** aus **`local_preview_result.json`**, falls vorhanden; ohne diese Datei (ältere Läufe) zeigen die Karten **UNKNOWN**. **BA 22.2** erlaubt das Öffnen/Einbetten von Preview, Report, OPEN_ME und JSON nur über **GET `/founder/dashboard/local-preview/file/{run_id}/{filename}`** (Whitelist, kein Zugriff außerhalb des jeweiligen **`local_preview_*`**-Ordners). Die Seite **GET `/founder/dashboard`** lädt das Panel per `fetch` im Browser; der **CLI-Flow** bleibt unverändert nutzbar.
