...
| Lokale Video-Produktion (Preview → Final MP4) | **Umgesetzt** unter **BA 20.x–32.x** (Orchestratoren, Local Preview, Production Pack, Founder Dashboard inkl. **Fresh Preview**-Cockpit und **URL→`final_video.mp4`** über **`run_ba265_url_to_final`** / **BA 32.3**); Runbooks: [docs/runbooks/real_video_build_wiring_map.md](docs/runbooks/real_video_build_wiring_map.md), [docs/runbooks/local_preview_render_v1.md](docs/runbooks/local_preview_render_v1.md), [docs/runbooks/founder_production_flow_v1.md](docs/runbooks/founder_production_flow_v1.md). **Cloud-Master-Render / Upload** bleiben Makro-**Phase 9/10** (siehe Phasenübersicht). |
...
| **BA 30.x–32.x** — Founder-Produktionslinie (Dashboard + lokale MP4) | **aktuelle Produktionsachse** | **BA 30.x–31.x:** Fresh Preview Smoke, Snapshot, Readiness, Operator Review, Guided Flow, Final-Render-Vorbereitung, Safe-Final-Handoff (read-only / CLI-Handoff). **BA 32.0:** Caps für längere Läufe (u. a. Timeline-Szenen, `max_live_assets`). **BA 32.1:** Runway **Live-Motion**-Checkbox + **422** ohne Key. **BA 32.62–32.69:** Motion-Slot-Plan, Runway-Hybrid-Chronik, bounded Playback/Fullscreen-Render und manuelles Live-2-Clip-Sicherheitsgate — siehe Tabelle **BA 32.x** und [docs/runbooks/real_video_build_wiring_map.md](docs/runbooks/real_video_build_wiring_map.md). **BA 32.3:** **`POST /founder/dashboard/video/generate`** → `output/video_generate/<run_id>/final_video.mp4` (Default u. a. **600 s** Ziel-Länge). **Zielbild nächster Ausbauschritt:** stabiler **10-Minuten**-Produktionslauf (**BA 32.0**; **BA 26.8** weiter **planned**) und weitere Operator-Inkremente. |
| **Phase 7** — Voiceover (TTS) Makro | **V1 done** | Produktions-API **`voice/synthesize-preview`** / **`voice/synthesize`** (OpenAI Speech), siehe Detailblock **Phase 7** und [docs/phases/phase7_voice_bauplan.md](docs/phases/phase7_voice_bauplan.md); optional **Baustein 7.6** zweiter Provider. |
...
| 7 | Voiceover | **done** (V1: siehe Detailblock **Phase 7**; optional **Baustein 7.6**) |
| 8 | Bild- / Szenenplan | **in progress** |
| 9 | Video Packaging | **in progress** (lokal: Preview/Final/MP4 über **BA 20.x–32.x** + FFmpeg; **kein** Ersatz für späteres Cloud-Mastering/Worker-Design) |
| 10 | Veröffentlichungsvorbereitung | **planned** |

**Hinweis zur Nummerierung:** Die **BA 9.x**-Bausteine (**Template Engine / Story Engine** in `app/story_engine/`) sind eine **eigene Produkt-Release-Linie** und **nicht** dasselbe wie **Phase 9** in dieser Tabelle (MP4/Packaging). Ausführlicher Bauplan: [PIPELINE_PLAN.md](PIPELINE_PLAN.md) (Abschnitt **„BA 9 — Template Engine / Story Engine (Produktachse)“**). **Phase 9** bezeichnet hier die **Makro**-Stufe „fertiges Paket/Export“; der **Ist-Stand lokaler MP4-Pipelines** (inkl. Copy-Final, Safe Final Render) ist in **BA 20.x–32.x** und den Runbooks **real_video_build_wiring_map**, **local_preview_render_v1**, **founder_production_flow_v1** beschrieben — ohne die offenen Architekturfragen (Batch-Worker vs. dedizierter Render-Service) vorwegzunehmen.
...
| | |
|--|--|
| **Hinweis** | **BA 9.x „Template Engine“** (Story-/Video-Format in `app/story_engine/`) ist eine **eigene Produktachse** und **nicht** identisch mit dieser klassischen **Phase 9** (MP4/Packaging). **BA 20.x–32.x** liefern den **lokalen** Verpackungs-/Render-Pfad (Preview-MP4, `final_video.mp4`, Handoffs) — siehe [docs/runbooks/real_video_build_wiring_map.md](docs/runbooks/real_video_build_wiring_map.md). |
| **Status** | **in progress** — **lokal:** wiederholbare MP4-Ketten (u. a. `render_final_story_video.py`, Local Preview, URL→Final über **`scripts/run_url_to_final_mp4.py`** / **BA 32.3**) **umgesetzt**; **Cloud:** reproduzierbarer Master-Build auf Cloud Run / Worker noch **offen** (Architekturentscheid). |
| **Ziel** | Schnitt, Untertitel, Branding, Export (z. B. MP4) — lokal oder Cloud-Job. |
| **Endpoints** | *teilweise lokal/CLI;* Dashboard: **`POST /founder/dashboard/video/generate`** (**BA 32.3**). |
| **Relevante Dateien** | u. a. `app/production_assembly/`, `scripts/run_url_to_final_mp4.py`, `app/founder_dashboard/ba323_video_generate.py`, Runbooks oben; FFmpeg lokal im `PATH`. |
| **Akzeptanzkriterien** | Reproduzierbarer Build; Ressourcenlimits Cloud Run beachten. |
| **Bekannte Grenzen** | Schwere Videoverarbeitung oft nicht auf kleinen Cloud-Run-Instanzen; **Runway-Live:** reale Zwei-Clip-Smokes bleiben operatorbestätigt, kostenbewusst und auf **`max_motion_clips=2`** begrenzt (**BA 32.69**); Last-/Kostenverhalten darüber hinaus ist nicht freigegeben. |
| **Nächster Schritt** | Architektur: Batch-Worker vs. dedizierter Render-Service (**Cloud**); lokal: optional **BA 26.8** (Longform-Smoke) und weitere Operator-Inkremente laut Tabelle **BA 30.x–32.x**. |
...
| BA 31.0 | Full Preview Smoke Review Loop (Operator Review) | **done** | Additiv in `fresh_preview_snapshot.py`: `evaluate_operator_review` — read-only Empfehlung `review_decision` (approve/rework/blocked/pending), `review_decision_label`, `review_decision_reasons`, `review_next_action`, `full_preview_available`, `preview_smoke_summary_ok`, `operator_review_version` (`ba31_0_v1`); keine Persistenz, keine Freigabe-Buttons. Merge nach Readiness; Snapshot-Version `ba31_0_v1`. Dashboard: Card **Operator Review** (`data-ba310-operator-review`, Badge `data-review-decision-marker`). Tests `tests/test_ba310_full_preview_review_loop.py` + Anpassungen `test_ba303`/`test_ba304`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 31.1 | Guided Production Flow (Founder Dashboard) | **done** | Additiv: `build_guided_production_flow` in `fresh_preview_snapshot.py` — `guided_flow_steps`, `guided_flow_next_step_label`, `guided_flow_next_step_action`, `guided_flow_current_step`; Merge nach Operator Review. Dashboard: Leiste **Production Flow** unter Executive Row (`data-ba311-guided-flow`, `#fd-guided-production-flow`). JS `fdFpApplyGuidedFlow` / Reset bei Snapshot-Fehler. Tests `tests/test_ba311_guided_production_flow.py`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 31.1b | Guided Flow Microcopy Clarification | **done** | Operatoren: Schritt **Snapshot** mit `detail` (Bezug **„Fresh Preview aktualisieren“**); klarerer Next-Step bei nur Dry-Run + Full-Preview-Pfad; statische Hilfezeile in der Production-Flow-Card; Handoff-Box: Hinweis nach CLI-Lauf. `guided_flow_version`: `ba31_1b_v1`. Keine API-/Routen-Änderung. Tests: `test_ba311_*`, `test_phase10_founder_dashboard.py`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 31.2 | Final Render Preparation Gate (Dashboard) | **done** | Additiv: `build_final_render_preparation_gate` in `fresh_preview_snapshot.py` — `final_render_gate_version` (`ba31_2_v1`), `final_render_gate_status` (locked/ready/blocked/needs_rework), Label, Gründe, `final_render_next_action`; Merge vor Guided Flow; Schritt **Final Render** im Guided Flow an Gate gekoppelt. UI: Card **Final Render Preparation** (`data-ba312-final-render-gate`). Kein Final-Render-Start, keine Provider-/Firestore-Writes. Tests `tests/test_ba312_final_render_preparation_gate.py` + Anpassungen `test_ba311`/`test_phase10`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 31.3 | Final Render Input Checklist (Dashboard) | **done** | Additiv: `build_final_render_input_checklist` — `final_render_input_checklist_version` (`ba31_3_v1`), Gesamtstatus, Items (Pfade, present/missing/unknown, `artifact_open_allowed`), `final_render_input_missing`, `final_render_input_next_action`; Merge nach Gate, vor Guided Flow. Artifact-Access erweitert (ohne neue Route): Root-JSONs `render_input_bundle_*`, `motion_timeline_manifest_*`, `motion_clip_manifest_*`, `local_preview_render_result.json` unter `.preview_smoke_work/.../local_preview/`. UI: **Final Render Input Checklist** (`data-ba313-final-render-input-checklist`). Tests `tests/test_ba313_final_render_input_checklist.py`, `test_ba309_*`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 31.4 | Safe Final Render Handoff (Dashboard) | **done** | Additiv: `build_safe_final_render_handoff` in `fresh_preview_snapshot.py` — `safe_final_render_handoff_version` (`ba31_4_v1`), `safe_final_render_handoff_available`, POSIX- und PowerShell-CLI für `scripts/run_safe_final_render.py` mit `--production-summary output/production_pack_<run_id>/production_summary.json` und `--output-dir output/safe_final_render_<run_id>` (kein `--execute`, keine Live-Provider-Flags); `safe_final_render_handoff_note` / `warning` / `reasons`. Verfügbar nur bei Gate **ready** und Input-Checkliste **ready** oder **warning** plus Kernartefakte und Run-ID. Merge nach Input-Checkliste, vor Guided Flow. UI: Karte **Safe Final Render Handoff** (`data-ba314-safe-final-render-handoff`, Kopier-Button). Kein Render-Start im Browser. Tests `tests/test_ba314_safe_final_render_handoff.py`, `test_phase10`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
| BA 32.0 | 10-Minute Production Blocker Removal (Leonardo cap + Timeline) | **done** | Optional `--max-live-assets` auf `run_fresh_topic_preview_smoke.py` → `max_live_assets` → Asset Runner `max_assets_live` (Default unverändert: implizit 3). `run_controlled_production_run` / `execute_preview_smoke_auto`: Parameter `max_timeline_scenes` (Default **5**); Fresh Preview Smoke reicht `max_scenes` als Timeline-Cap durch. CLI: `run_preview_smoke_auto.py`, `run_first_real_production_30_60.py` mit `--max-timeline-scenes`. Keine UI-Änderung. Tests `tests/test_ba320_ten_minute_blocker_removal.py`. |
| BA 32.1 | Runway Live Motion — Readiness & Blocking (kein Fake-Live) | **done** | **`POST /founder/dashboard/video/generate`**: Live-Flags verlangen **`confirm_provider_costs`** (sonst **422**); **`allow_live_motion`** ohne **`RUNWAY_API_KEY`** → **422** `live_motion_requires_runway_connector`. |
| BA 32.62 | Motion Slot Planner (Zeitfenster im `run_summary`) | **done** | `build_motion_slots` / `motion_slot_plan` vor Timeline; keine Provider-Calls. |
| BA 32.63 | Runway — erster Motion-Slot → `video_path` | **done** | Nach Fit-to-Voice, vor Timeline: optional ein Runway Image-to-Video-Clip für Slot 1 bei `max_motion_clips` ≥ 1 + Key; `motion_clip_artifact`. Tests u. a. `test_ba363`. |
| BA 32.64 | Motion Readiness vs. gerenderte Clips | **done** | `readiness_audit`: `motion_requested` / `motion_rendered` / `motion_ready`; kein `live_motion_not_available`-Blocker bei erfolgreichem Render. Tests `test_ba323`. |
| BA 32.65 | Erster erfolgreicher Runway Hybrid Smoke (Doku) | **done** | Chronik `run_id` `video_gen_10m_1778273622251` — Runway + Gemini + ElevenLabs, `production_ready`, ohne Placeholder; Runbooks: `real_video_build_wiring_map.md`, `founder_production_flow_v1.md`, Querverweis `real_image_provider_smoke.md`. |
| BA 32.66 | Bounded Motion Clip Playback Window | **done** | Motion-Clips werden im Render nur im Slot-Fenster abgespielt; längere Szenen fallen auf Bildsegment zurück statt den Clip über die volle Szene zu loopen. Tests: `tests/test_ba366_motion_clip_playback_window.py`. |
| BA 32.68 | Fullscreen/Cover Render for Hybrid Segments | **done** | 16:9-Cover-Verhalten für Bild- und Video-Segmente ohne Letterboxing; Hybrid-Render bleibt bounded und lokal. Tests: `tests/test_ba368_fullscreen_image_render.py`. |
| BA 32.69 | Optional Live 2-Clip Smoke Checklist | **done** | Dokumentations-first Sicherheitsgate für genau einen manuell bestätigten Live-Zwei-Clip-Smoke: `docs/runbooks/live_2_clip_smoke_checklist.md`; optionaler Dry-Run-Readiness-Helper `scripts/check_live_2_clip_smoke_readiness.py` prüft nur ENV-Präsenz/Operator-Bestätigungen, keine Provider-Calls, keine Secrets. Tests: `tests/test_ba3269_live_2_clip_smoke_readiness.py`. |
| BA 32.70 | Founder Dashboard Visual Redesign V1 | **done** | UI-only Refresh des Founder Cockpits nach `docs/design/reference/dashboard_mockup.png` / `video_pipeline_dashboard_reference.html`: modernere SaaS-Cockpit-Hierarchie, Executive Cards, Video-Generate Operator-Brief, Produktionsgates, klarere Readiness-/Artifact-/Warning-Gruppierung. Keine Backend-, Provider- oder Render-Logik geändert; bestehende IDs/Aktionen bleiben erhalten. Tests: `tests/test_phase10_founder_dashboard.py`, `tests/test_ba323_dashboard_video_generate.py`. |
| BA 32.90 | Founder Dashboard Visual Skin V1 | **done** | UI-only Skin in `app/founder_dashboard/html.py`: Design-Tokens, dunkle App-Shell, hochwertigere Sidebar, Executive-Cards, Panels, Buttons, Status-Badges, JSON-/Output-Flächen und Next-Step-Bereiche näher an `docs/design/reference/dashboard_mockup.png`. Kontrollierter CSS-Override mit `data-ba3290-visual-skin`, keine API-/Provider-/JS-Logik geändert, bestehende IDs/Aktionen bleiben erhalten. Tests: `python -m compileall app`, `python -m pytest tests/test_phase10_founder_dashboard.py`. |
| BA 32.92 | Founder Dashboard Video Generate Form Polish | **done** | UI-only in `app/founder_dashboard/html.py`: Video-Generate-Primärformular auf URL, Dauer, Image-Provider, Voice, Live-/Kostenkontrollen und Haupt-CTA fokussiert; technische Szenen-/Motion-/OpenAI-/Thumbnail-Parameter in `details#fd-vg-advanced-production-params` verschoben. IDs, Actions, JS-Hooks und Backend-Verträge bleiben unverändert; keine Provider-/Render-Logik geändert. Tests: `python -m compileall app scripts tests`, `python -m pytest tests/test_phase10_founder_dashboard.py -q`, `python -m pytest tests/test_ba323_dashboard_video_generate.py -q`, `python -m pytest`. |
| BA 32.3 | Dashboard URL-to-Video Generation Flow | **done** | Panel „Video generieren“; `POST /founder/dashboard/video/generate` → `run_ba265_url_to_final` unter `output/video_generate/<run_id>/`. Motion-Metadaten im Pack; optional Runway-Slot (**BA 32.63**); Readiness **BA 32.64**. Live-Bilder: `asset_runner_mode=live` + `max_live_assets`. Tests `tests/test_ba323_dashboard_video_generate.py`. Runbooks: `founder_production_flow_v1.md`, `local_preview_render_v1.md`, `real_video_build_wiring_map.md`. |
...
| BA 26.4R | Real Provider Smoke Audit & Next-Step | **done** (Doku) | Audit + Entscheidungshilfe **BA 26.5** (URL-to-Final); siehe Abschnitt **BA 26.4R** unter **BA 26.x — Real Video Build**. |
| BA 26.5 | URL-to-final-MP4 Founder Run (`run_ba265_url_to_final`) | **done** | [`scripts/run_url_to_final_mp4.py`](scripts/run_url_to_final_mp4.py): **URL** oder **`script.json`** → Skript/Pack → `run_asset_runner` (placeholder/live; inkl. **OpenAI Images** mit dry-run default, Live nur mit Flag/ENV — Tests `tests/test_ba265_openai_images_adapter.py`) → Timeline → **`final_video.mp4`** + `run_summary.json` / `render_result.json`; optional **`--asset-dir`** für lokale Clips. Direkter Pfad **URL → MP4**; Dashboard **BA 32.3** delegiert hierher. Tests u. a. `tests/test_ba265_url_to_final_mp4.py`. |
...
Erster echter Provider-Pfad für die Storyboard-Kette: `POST /story-engine/openai-image-live-execution` führt maximal einen `image`-Task aus einem `AssetGenerationPlan` über den bestehenden OpenAI-Image-Connector aus. Erfordert `confirm_provider_costs=true`, ist auf `max_live_image_tasks=1` begrenzt, nutzt standardmäßig `gpt-image-2` und `1024x1024`, und schreibt das Bild nach `output/storyboard_runs/<run_id>/<scene_id>/image.png`. Dashboard ergänzt den manuellen Button `OpenAI Bild erzeugen` und Checkbox `OpenAI Image Kosten bestätigen`; der automatische Full-Pipeline-Button bleibt plan-only/safe und startet diesen Live-Call nicht. Keine Firestore-Writes, keine Änderung am `GenerateScriptResponse`-Vertrag. Tests mocken den Connector: `tests/test_openai_image_live_execution.py`, `tests/test_phase10_founder_dashboard.py`.
