...
| **Bekannte Grenzen** | Schwere Videoverarbeitung oft nicht auf kleinen Cloud-Run-Instanzen; **Runway:** in **`run_ba265_url_to_final`** derzeit bewusst **höchstens zwei** Motion-Slot-Clips (**BA 32.66**); längere Mehrfach-Clips / Last / Kosten bleiben offen (**BA 32.68+**). |
...
| BA 32.66 | 2-Clip Hybrid Smoke (Mock-first) | **done** | `runway_motion_integration` rendert bounded bis zu **2** Slots (`max_motion_clips=1` bleibt First-Slot; `0`/kein Key safe). `motion_clip_artifact`: planned/attempted/rendered/failed/skipped, `video_clip_paths`, `slot_results`; Timeline konsumiert zwei `video_path`-Szenen. Tests: `test_ba363`, `test_ba366`. |
| BA 32.67 | Multi-Clip Status & Readiness Reporting | **done** | Dashboard/OPEN_ME zeigen Motion-Zähler: requested/attempted/rendered/failed/skipped sowie `motion_used_real_clips` / `motion_used_placeholders`; keine breite UI-Umgestaltung. Tests: `test_ba323_dashboard_video_generate.py`. |
| BA 32.68 | Documentation Sync | **done** | Runbook und Roadmap dokumentieren Ein-Clip-/Zwei-Clip-Verhalten, Provider-Safety, Mock-vs.-Live-Smoke und Restrisiken vor längeren Produktionsläufen. |
| BA 32.3 | Dashboard URL-to-Video Generation Flow | **done** | Panel „Video generieren“; `POST /founder/dashboard/video/generate` → `run_ba265_url_to_final` unter `output/video_generate/<run_id>/`. Motion-Metadaten im Pack; optional bis zu zwei Runway-Slots (**BA 32.66**, capped); Readiness/Status **BA 32.64/32.67**. Live-Bilder: `asset_runner_mode=live` + `max_live_assets`. Tests `tests/test_ba323_dashboard_video_generate.py`. Runbooks: `founder_production_flow_v1.md`, `local_preview_render_v1.md`, `real_video_build_wiring_map.md`. |
