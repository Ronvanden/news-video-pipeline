...
| **BA 30.x–32.x** — Founder-Produktionslinie (Dashboard + lokale MP4) | **aktuelle Produktionsachse** | **BA 30.x–31.x:** Fresh Preview Smoke, Snapshot, Readiness, Operator Review, Guided Flow, Final-Render-Vorbereitung, Safe-Final-Handoff (read-only / CLI-Handoff). **BA 32.0:** Caps für längere Läufe (u. a. Timeline-Szenen, `max_live_assets`). **BA 32.1:** Runway **Live-Motion** nur Readiness/Blocking + ehrliche Warnungen — **keine** Runway-Clip-Erzeugung in **`run_ba265_url_to_final`** (siehe Detailzeile **BA 32.1**). **BA 32.3:** **`POST /founder/dashboard/video/generate`** → `output/video_generate/<run_id>/final_video.mp4` (Default u. a. **600 s** Ziel-Länge). **Zielbild nächster Ausbauschritt:** URL → Video-Generate → stabiler **10-Minuten**-Produktionslauf (Teil bereits über Defaults + **BA 32.0**; **BA 26.8** weiter **planned**). |
...
| **Bekannte Grenzen** | Schwere Videoverarbeitung oft nicht auf kleinen Cloud-Run-Instanzen; **Runway Live-Motion** ist in **`run_ba265_url_to_final`** / **BA 32.3** **nicht** als vollständige Clip-Erzeugung integriert (nur Readiness/422/Warnung, siehe **BA 32.1**). |
...
| BA 32.1 | Runway Live Motion — Readiness & Blocking (kein Fake-Live) | **done** | Umsetzung in **`POST /founder/dashboard/video/generate`** + `execute_dashboard_video_generate` (**kein** separater Endpunkt): Live-Asset/Motion-Flags verlangen **`confirm_provider_costs`** (sonst **422** `confirm_provider_costs_required_when_live_flags`); **`allow_live_motion`** ohne nicht-leeres **`RUNWAY_API_KEY`** → **422** `live_motion_requires_runway_connector`. Mit Key erzeugt **`run_ba265_url_to_final`** **keine** Runway-Clips — nur Metadaten + **Warnung** `ba323_live_motion_runway_configured_but_url_to_final_does_not_ingest_runway_clips` (siehe `app/founder_dashboard/ba323_video_generate.py`). |
| BA 32.3 | Dashboard URL-to-Video Generation Flow | **done** | Eigenes Panel „Video generieren“ (`panel-ba323-video-generate`) vor Fresh Preview; `POST /founder/dashboard/video/generate` ruft `run_ba265_url_to_final` (Wrapper `app/founder_dashboard/ba323_video_generate.py`) unter `output/video_generate/<run_id>/` auf. Defaults: 600 s, 24 Szenen, Motion-Metadaten (alle 60 s, max 10 Clips, Dauer 10 s) im Pack-Metadata; **keine** Runway-Clip-Erzeugung in **`run_ba265_url_to_final`** — Live-Motion nur mit `RUNWAY_API_KEY`-Präsenz + Kostenbestätigung, sonst 422; Warnung wenn Key da, Pipeline aber keine Runway-Ingest. Live-Bilder: `asset_runner_mode=live` + `max_live_assets`. Tests `tests/test_ba323_dashboard_video_generate.py`; Dashboard-Tests erweitert. Runbooks: `founder_production_flow_v1.md`, `local_preview_render_v1.md`, `real_video_build_wiring_map.md`. |
