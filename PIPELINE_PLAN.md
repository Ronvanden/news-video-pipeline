...
| BA 32.0 | 10-Minute Production Blocker Removal (Leonardo cap + Timeline) | **done** | Optional `--max-live-assets` auf `run_fresh_topic_preview_smoke.py` → `max_live_assets` → Asset Runner `max_assets_live` (Default unverändert: implizit 3). `run_controlled_production_run` / `execute_preview_smoke_auto`: Parameter `max_timeline_scenes` (Default **5**); Fresh Preview Smoke reicht `max_scenes` als Timeline-Cap durch. CLI: `run_preview_smoke_auto.py`, `run_first_real_production_30_60.py` mit `--max-timeline-scenes`. Keine UI-Änderung. Tests `tests/test_ba320_ten_minute_blocker_removal.py`. Runway Live bleibt BA 32.1. |
| BA 32.1 | Runway Live Readiness / Pipeline Motion Stub Klärung | **done** | Pipeline bleibt Dry-Run/Stub (`RUNWAY_LIVE_CONNECTOR_IMPLEMENTED=False`); echte Runway-Einzelclips weiter über `scripts/runway_image_to_video_smoke.py`. `motion_clip_manifest.summary`: `runway_live_available`, `runway_live_reason`, `pipeline_motion_mode`, optional `global_warnings`. `--allow-live-motion` + `--max-live-motion-clips` auf `run_fresh_topic_preview_smoke.py`, `run_preview_smoke_auto.py`, `run_first_real_production_30_60.py`; kontrolliertes Blocking `runway_live_motion_requested_but_connector_not_implemented`. `build_motion_clip_result` annotiert Live-Wunsch ohne Fake-Live. Tests `tests/test_ba321_runway_live_readiness.py`. Keine UI. |
| BA 32.2 | ElevenLabs Longform Voice Completeness | **done** | `scripts/build_full_voiceover.py`: parametrisierbare Chunk-Größe (`--elevenlabs-chunk-max-chars`, Env `ELEVENLABS_CHUNK_MAX_CHARS`) für lange Narration; Manifest-Felder `voice_completeness`, Blocking bei fehlendem API-Key / `strict_live_voice` statt stiller Smoke-Ersatz für elevenlabs/openai; `--strict-live-voice`. `controlled_production_run`: Voice-Manifest in Production Pack / Render Input Bundle (`voice_manifest`, `voice_paths`, Merge von Warnings/Blocking); `--voice-manifest` auf Preview-Smoke / First-Real CLIs. Tests: `tests/test_build_full_voiceover.py`, `tests/test_ba322_voice_longform_bundle.py`. Keine UI. |
