...
| BA 31.1 | Guided Production Flow (Founder Dashboard) | **done** | Additiv: `build_guided_production_flow` in `fresh_preview_snapshot.py` — `guided_flow_version` (`ba31_1_v1`), `guided_flow_steps`, `guided_flow_next_step_label`, `guided_flow_next_step_action`, `guided_flow_current_step`; Merge nach Operator Review. Dashboard: Leiste **Production Flow** unter Executive Row (`data-ba311-guided-flow`, `#fd-guided-production-flow`). JS `fdFpApplyGuidedFlow` / Reset bei Snapshot-Fehler. Tests `tests/test_ba311_guided_production_flow.py`. Runbooks: `local_preview_render_v1.md`, `founder_production_flow_v1.md`. |
